...
```
This results in a mean of `r meanSteps` and a median of `r medSteps`.
...
```
This results in the `r maxMeanInterval` interval as having the maximum mean of steps, averaged over all days.
...
```
This results in **`r countMissing`** rows with missing data.
...
The total number of replaced missing values was **`r count`**.
...
```
This results in a mean of `r impMeanSteps` and a median of `r impMedSteps`.
